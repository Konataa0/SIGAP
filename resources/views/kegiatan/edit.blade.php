...
<div class="max-w-3xl space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h3 class="text-xl font-extrabold text-white">Edit Kegiatan</h3>
            <p class="text-sm text-slate-400">{{ $kegiatan->nama }}</p>
        </div>
    </div>

    <form method="POST" action="{{ route('admin.kegiatan.update', $kegiatan) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select name="jenis"
                        ...
                        <option value="lomba"       {{ old('jenis', $kegiatan->jenis) === 'lomba'       ? 'selected' : '' }}>Lomba / Kompetisi</option>
                        <option value="sertifikasi" {{ old('jenis', $kegiatan->jenis) === 'sertifikasi' ? 'selected' : '' }}>Sertifikasi</option>
                    </select>
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5">Penyelenggara</label>
                    <input type="text" name="penyelenggara" value="{{ old('penyelenggara', $kegiatan->penyelenggara) }}"
                           class="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-4 py-2.5 text-sm text-white outline-none transition-all">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="deskripsi" rows="3"
                              class="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-4 py-2.5 text-sm text-white outline-none transition-all resize-none">{{ old('deskripsi', $kegiatan->deskripsi) }}</textarea>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="syarat_ketentuan" rows="3"
                              class="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-4 py-2.5 text-sm text-white outline-none transition-all resize-none">{{ old('syarat_ketentuan', $kegiatan->syarat_ketentuan) }}</textarea>
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5">Deadline Pendaftaran</label>
                    <input type="date" name="deadline_pendaftaran" value="{{ old('deadline_pendaftaran', optional($kegiatan->deadline_pendaftaran)->format('Y-m-d')) }}"
                           class="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-4 py-2.5 text-sm text-white outline-none transition-all">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5">Link Pendaftaran</label>
                    <input type="url" name="link_pendaftaran" value="{{ old('link_pendaftaran', $kegiatan->link_pendaftaran) }}"
                           class="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-4 py-2.5 text-sm text-white outline-none transition-all">
                </div>

                <div class="md:col-span-2">
                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5">Kontak PIC</label>
                    <input type="text" name="kontak_pic" value="{{ old('kontak_pic', $kegiatan->kontak_pic) }}"
                           class="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-4 py-2.5 text-sm text-white outline-none transition-all">
                </div>
            </div>
        </div>

        {{-- Nilai Kriteria (terisi otomatis dari DB) --}}
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-5">
            <div>
                <h4 class="text-sm font-bold text-slate-300 uppercase tracking-wider pb-2 border-b border-slate-800">
                    Nilai Kriteria SAW
                </h4>
                <p class="text-xs text-slate-500 mt-2">Nilai yang sudah tersimpan sebelumnya ditampilkan sebagai pilihan default.</p>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @foreach($kriterias as $k)
                @php
                    $nilaiSaatIni = old("nilai.{$k->id}", $nilaiExisting[$k->id] ?? '');
                @endphp
                <div class="p-4 bg-slate-950 border border-slate-800 rounded-xl space-y-2">
                    <div class="flex items-center justify-between">
                        <label class="text-sm font-bold text-white">{{ $k->kode }} — {{ $k->nama }}</label>
                    ...
                    <select name="nilai[{{ $k->id }}]"
                        ...
                        @for($i = 1; $i <= 5; $i++)
                            <option value="{{ $i }}" {{ $nilaiSaatIni == $i ? 'selected' : '' }}>
        ...
        <div class="flex items-center gap-3 justify-end">
            ...
            <button type="submit"
                ...
                <i data-lucide="save" class="w-4 h-4"></i> Perbarui Kegiatan
            </button>
        </div>
    </form>
